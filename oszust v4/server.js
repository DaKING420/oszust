...
function publicState(room, viewerId) {
  ...
  return {
    ...
    game: game ? {
      ...
      canDraw: !game.winner && game.turn === room.players.findIndex(p => p.id === viewerId) && game.drawPile.length >= 3,
...
function startGame(room, minRank, maxRank) {
  ...
  // Zostawiamy osobny stos dobierania. Dzięki temu DOBIERZ 3 może
  // zostać użyte wielokrotnie w trakcie całej gry, dopóki są co najmniej
  // 3 karty. Wielkość stosu zależy od liczby graczy, ale nigdy nie zabiera
  // całej talii przy większej liczbie graczy.
  const reserveSize = Math.min(12, Math.max(6, room.players.length * 3), available.length);
  const drawPile = available.slice(0, reserveSize);

  // 9♥ (lub inna wybrana najniższa karta kier) ma być rozdawana LOSOWEMU
  // graczowi. Nie może jednak trafić do stosu dobierania. Wstawiamy ją więc
  // w losowe miejsce wśród kart przeznaczonych do rozdania.
  const dealDeck = available.slice(reserveSize);
  const starterInsertIndex = Math.floor(Math.random() * (dealDeck.length + 1));
  dealDeck.splice(starterInsertIndex, 0, starterCard);
...
wss.on('connection', ws => {
  ...
  ws.on('message', raw => {
    ...
    if (data.type === 'draw3') {
      if (game.winner) return error(ws, 'Gra jest już zakończona.');
      // Dobieranie oznacza rezygnację ze sprawdzania poprzedniej deklaracji.
      // Najpierw sprawdzamy turę, dopiero potem zamykamy możliwość pukania.
      if (game.turn !== myIndex) return error(ws, 'To nie jest Twoja tura.');
      game.pendingClaim = null;
      ...
      // Następna karta pozostaje na WIERZCHU stosu. Tylko ją odkrywamy —
      // nie zdejmujemy jej z talii. Dzięki temu wszyscy widzą dokładnie
      // tę kartę, która faktycznie leży na górze i wyznacza dalszą rangę.
      // Przy kolejnym DOBIERZ 3 ta sama odkryta karta będzie pierwszą
      // dobieraną kartą, a po niej zostanie odkryta następna.
      if (game.drawPile.length > 0) {
        game.visibleCard = game.drawPile[0];
    ...
    if (data.type === 'knock') {
      ...
      // Zapamiętujemy kolejkę PRZED rozpatrzeniem pukania. Pukanie jest
      // niezależne od kolejki i samo w sobie nie może przypadkiem przesunąć
      // tury na kolejnego gracza.
      const turnBeforeKnock = game.turn;
      const turnPlayerId = room.players[turnBeforeKnock]?.id;
      const knockerIndex = room.players.findIndex(p => p.id === me.id);
      const wasTurnPlayer = turnPlayerId === me.id;

      // Kolejka zmienia się tylko wtedy, gdy PUKA osoba, która właśnie ma
      // kolejkę. Jeśli jej pukanie jest trafione, zostaje na swojej kolejce.
      // Jeśli jest pudło, kolejka przechodzi na osobę NASTĘPNĄ po pukającym.
      // Pukanie osoby spoza kolejki nie może zmienić kolejki w ogóle.
      if (!wasTurnPlayer) {
        game.turn = turnBeforeKnock;
      } else if (!truth) {
        // PUDŁO: aktualny gracz traci kolejkę. Liczymy następnego gracza
        // bezpośrednio z indeksu pukającego, np. A -> B -> C -> A.
        game.turn = nextIndex(room, knockerIndex);
      } else {
        // TRAFIONE: oszust zbiera stos, ale pukający zachowuje kolejkę.
        game.turn = knockerIndex;
      }
