...
wss.on('connection', ws => {
  ...
  ws.on('message', raw => {
    ...
    if (data.type === 'knock') {
      ...
      if (truth) {
        addLog(game, `🔔 ${me.name}: PUKANIE TRAFIONE — ${target.name} oszukał(a) i zbiera stos.`);
      } else {
        addLog(game, `🔔 ${me.name}: PUKANIE PUDŁO — ${target.name} mówił(a) prawdę. ${me.name} zbiera stos i traci turę.`);
      }
