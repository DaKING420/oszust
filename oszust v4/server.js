...
wss.on('connection', ws => {
  ...
  ws.on('message', raw => {
    ...
    if (data.type === 'knock') {
      ...
      // Jeśli puka aktualny gracz:
      // - trafione pukanie: zachowuje swoją kolejkę,
      // - pudło: traci kolejkę i gra następny.
      // Jeśli puka gracz spoza kolejki, wynik pukania NIGDY nie zmienia kolejki.
      if (wasTurnPlayer && truth) {
        // Trafione pukanie aktualnego gracza nie zabiera mu kolejki.
        game.turn = turnBeforeKnock;
      } else if (wasTurnPlayer && !truth) {
        // Pudło aktualnego gracza zabiera mu kolejkę. Następny gracz jest
        // liczony od AKTUALNEGO pukającego, a nie od indeksu użytego do
        // wcześniejszej deklaracji. Dzięki temu np. C -> A, a nie C -> C.
        game.turn = nextIndex(room, myIndex);
      } else {
        // Pukanie gracza spoza kolejki nigdy nie zmienia kolejki.
        game.turn = turnBeforeKnock;
      }
