...
wss.on('connection', ws => {
  ...
  ws.on('message', raw => {
    ...
    if (data.type === 'knock') {
      ...
      // Pukanie NIGDY nie zmienia kolejki. Po rozstrzygnięciu nadal jedzie
      // gracz, który był następny po ostatnim zagraniu.
      if (truth) {
        addLog(game, `🔔 ${me.name}: PUKANIE TRAFIONE — ${target.name} oszukał(a). ${target.name} zbiera stos. Kolejka nadal: ${room.players[game.turn].name}.`);
      } else {
        addLog(game, `🔔 ${me.name}: PUKANIE PUDŁO — ${target.name} mówił(a) prawdę. ${me.name} zbiera stos. Kolejka nadal: ${room.players[game.turn].name}.`);
      }
