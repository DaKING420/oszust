  ...
  ws.on('message', raw => {
    ...
    if (data.type === 'play') {
      ...
      if (me.hand.length === 0) {
        ...
        game.placements.push({ playerId: me.id, name: me.name, place, bluffCount: me.bluffCount || 0 });
        game.activeIds = game.activeIds.filter(id => id !== me.id);
        game.pendingClaim = null;
        addLog(game, `🏆 ${me.name} zajmuje ${place}. miejsce!`);
