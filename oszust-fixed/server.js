...
wss.on('connection', ws => {
  ...
  ws.on('message', raw => {
    ...
    if (data.type === 'play') {
      ...
      // Po każdym normalnym zagraniu tura przechodzi na następną osobę.
      game.turn = nextIndex(room, myIndex);

      console.log(`🃏 PLAY ${me.name}: ${cards.length}x ${claimedRank}, pokój ${room.id}; następna tura: ${room.players[game.turn].name}`);
      broadcast(room);
      return;
    }
    ...
    if (data.type === 'knock') {
      ...
      // Jeżeli pukający robi to w swojej turze, pukanie zużywa tę turę.
      // Jeżeli puka poza swoją turą, aktualna tura pozostaje bez zmian.
      const knockWasMyTurn = game.turn === myIndex;
      if (knockWasMyTurn) game.turn = nextIndex(room, myIndex);

      if (truth) {
        addLog(game, `${me.name} zapukał(a) — miał(a) rację. ${target.name} zbiera stos.`);
      } else {
        addLog(game, `${me.name} zapukał(a) — pudło. ${me.name} zbiera stos i traci swoją turę.`);
      }
